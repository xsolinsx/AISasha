...
local function run(msg, matches)
    if is_sudo(msg) then
        ...
        set_text('requireSignUp', 'Before dying you need to be registered, use /registrami.')
        ...
        -- google.lua --
        set_text('google:0', 2)
        set_text('google:1', '(#google|[sasha] googla) <terms>: Sasha googles <terms>.')
        ...
        set_text('ingroup:21', '(#setlink|[sasha] imposta link): Sasha saves a group\'s link with the one that will be sent.')
        ...
        set_text('ingroup:31', '#kill chat|realm: Sasha kicks every user in group|realm and removes it.')
        ...
        set_text('inrealm:18', '#kill chat <group_id>: Sasha kicks all members of <group_id> and removes <group_id>.')
        set_text('inrealm:19', '#kill realm <realm_id>: Sasha kicks all members of <realm_id> and removes <realm_id>.')
        ...
        set_text('stats:4', '(#statslist|#messageslist): Sasha sends file with chat\'s stats.')
        set_text('stats:5', 'ADMIN')
        set_text('stats:6', '(#stats|#messages) group <group_id>: Sasha sends <group_id>\'s stats.')
        set_text('stats:7', '(#statslist|#messageslist) group <group_id>: Sasha sends file with <group_id>\'s stats.')
        set_text('stats:8', '(#stats|#messages) aisasha: Sasha sends her stats.')
        ...
        set_text('supergroup:25', '(#setlink|sasha imposta link): Sasha saves a group\'s link with the one that will be sent.')
        ...
        set_text('welcome:1', '#getwelcome: Sasha sends welcome.')
        set_text('welcome:2', 'OWNER')
        set_text('welcome:3', '#setwelcome <text>: Sasha sets <text> as welcome.')
        set_text('welcome:4', '#setmemberswelcome <value>: Sasha after <value> members will send welcome, if zero welcome will not be sent anymore.')
        set_text('welcome:5', '#getmemberswelcome: Sasha sends value of users that are needed to get welcome.')
