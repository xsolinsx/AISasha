local function get_rank_by_username(extra, success, result)
    ...
    send_large_msg(extra.receiver, langs['it'].rank .. reverse_rank_table[rank + 1])
end
...
local function callback_group_members(extra, success, result)
    ...
    local text = langs['it'].usersIn .. string.gsub(chatname, "_", " ") .. ' ' .. result.peer_id .. '\n'
...
local function channel_callback_ishere(extra, success, result)
    ...
    local text = langs['it'].ishereNo
    for k, v in pairsByKeys(result) do
        if tonumber(user) then
            if tonumber(v.peer_id) == tonumber(user) then
                text = langs['it'].ishereYes
            end
...
local function info_by_username(extra, success, result)
    if success == 0 then
        send_large_msg(extra.receiver, langs['it'].noUsernameFound)
        return
    end
    local text = langs['it'].info .. ' (<username>)'
    if result.peer_type == 'channel' then
        if result.title then
            text = text .. langs['it'].name .. result.title
        end
    ...
    elseif result.peer_type == 'user' then
        ...
        text = text .. langs['it'].rank .. reverse_rank_table[get_rank(result.peer_id, extra.chat_id) + 1] ..
        langs['it'].date .. os.date('%c') ..
        ...
        text = langs['it'].peerTypeUnknown
    end
    send_large_msg(extra.receiver, text)
end

local function info_by_reply(extra, success, result)
    local text = langs['it'].info .. ' (<reply>)'
    local action = false
    ...
    if action and result.action.user then
        if result.action.user.first_name then
            text = text .. langs['it'].name .. result.action.user.first_name
        end
        if result.action.user.real_first_name then
            text = text .. langs['it'].name .. result.action.user.real_first_name
        end
        if result.action.user.last_name then
            text = text .. langs['it'].surname .. result.action.user.last_name
        end
        if result.action.user.real_last_name then
            text = text .. langs['it'].surname .. result.action.user.real_last_name
        end
        if result.action.user.username then
            text = text .. langs['it'].username .. '@' .. result.action.user.username
        end
        -- exclude bot phone
        if our_id ~= result.action.user.peer_id then
            --[[
            if result.action.user.phone then
                text = text .. langs['it'].phone .. '+' .. string.sub(result.action.user.phone, 1, 6) .. '******'
            end
            ]]
        end
        local msgs = tonumber(redis:get('msgs:' .. result.action.user.peer_id .. ':' .. result.to.peer_id) or 0)
        text = text .. langs['it'].rank .. reverse_rank_table[get_rank(result.action.user.peer_id, result.to.peer_id) + 1] ..
        langs['it'].date .. os.date('%c') ..
        langs['it'].totalMessages .. msgs ..
        langs['it'].otherInfo
        ...
        if result.from.first_name then
            text = text .. langs['it'].name .. result.from.first_name
        end
        if result.from.real_first_name then
            text = text .. langs['it'].name .. result.from.real_first_name
        end
        if result.from.last_name then
            text = text .. langs['it'].surname .. result.from.last_name
        end
        if result.from.real_last_name then
            text = text .. langs['it'].surname .. result.from.real_last_name
        end
        if result.from.username then
            text = text .. langs['it'].username .. '@' .. result.from.username
        end
        -- exclude bot phone
        if our_id ~= result.from.peer_id then
            --[[
            if result.from.phone then
                text = text .. langs['it'].phone .. '+' .. string.sub(result.from.phone, 1, 6) .. '******'
            end
            ]]
        end
        local msgs = tonumber(redis:get('msgs:' .. result.from.peer_id .. ':' .. result.to.peer_id) or 0)
        text = text .. langs['it'].rank .. reverse_rank_table[get_rank(result.from.peer_id, result.to.peer_id) + 1] ..
        langs['it'].date .. os.date('%c') ..
        langs['it'].totalMessages .. msgs ..
        langs['it'].otherInfo
...
local function info_by_from(extra, success, result)
    local text = langs['it'].info .. ' (<from>)'
    if result.fwd_from.peer_type == 'channel' then
        if result.fwd_from.title then
            text = text .. langs['it'].name .. result.fwd_from.title
        end
        if result.fwd_from.username then
            text = text .. langs['it'].username .. '@' .. result.fwd_from.username
        end
        text = text .. langs['it'].date .. os.date('%c') ..
        '\n🆔: ' .. result.fwd_from.peer_id
    elseif result.fwd_from.peer_type == 'user' then
        if result.fwd_from.first_name then
            text = text .. langs['it'].name .. result.fwd_from.first_name
        end
        if result.fwd_from.real_first_name then
            text = text .. langs['it'].name .. result.fwd_from.real_first_name
        end
        if result.fwd_from.last_name then
            text = text .. langs['it'].surname .. result.fwd_from.last_name
        end
        if result.fwd_from.real_last_name then
            text = text .. langs['it'].surname .. result.fwd_from.real_last_name
        end
        if result.fwd_from.username then
            text = text .. langs['it'].username .. '@' .. result.fwd_from.username
        end
        ...
        if our_id ~= result.fwd_from.peer_id then
            ...
            if result.fwd_from.phone then
                text = text .. langs['it'].phone .. '+' .. string.sub(result.fwd_from.phone, 1, 6) .. '******'
            end
            ]]
        end
        local msgs = tonumber(redis:get('msgs:' .. result.fwd_from.peer_id .. ':' .. result.to.peer_id) or 0)
        text = text .. langs['it'].rank .. reverse_rank_table[get_rank(result.fwd_from.peer_id, result.to.peer_id) + 1] ..
        langs['it'].date .. os.date('%c') ..
        langs['it'].totalMessages .. msgs ..
        langs['it'].otherInfo
...
local function channel_callback_info(extra, success, result)
    local title = langs['it'].supergroupName .. result.title
    local user_num = langs['it'].users .. tostring(result.participants_count)
    local admin_num = langs['it'].admins .. tostring(result.admins_count)
    local kicked_num = langs['it'].kickedUsers .. tostring(result.kicked_count)
    local channel_id = "\n🆔: " .. result.peer_id
    if result.username then
        channel_username = langs['it'].username .. "@" .. result.username
    else
        channel_username = ""
    end
...
local function chat_callback_info(extra, success, result)
    local title = langs['it'].groupName .. result.title
    local user_num = langs['it'].users .. tostring(result.members_num)
    local chat_id = "\n🆔: " .. result.peer_id
...
local function pre_process(msg)
    if msg.to.type == 'user' and msg.fwd_from then
        if is_support(msg.from.id) or is_admin1(msg) then
            local text = langs['it'].info .. ' (<private_from>)'
            if msg.fwd_from.peer_type == 'channel' then
                if msg.fwd_from.title then
                    text = text .. langs['it'].name .. msg.fwd_from.title
                end
                if msg.fwd_from.username then
                    text = text .. langs['it'].username .. '@' .. msg.fwd_from.username
                end
                text = text .. langs['it'].date .. os.date('%c') ..
                '\n🆔: ' .. msg.fwd_from.peer_id
            elseif msg.fwd_from.peer_type == 'user' then
                if msg.fwd_from.first_name then
                    text = text .. langs['it'].name .. msg.fwd_from.first_name
                end
                if msg.fwd_from.real_first_name then
                    text = text .. langs['it'].name .. msg.fwd_from.real_first_name
                end
                if msg.fwd_from.last_name then
                    text = text .. langs['it'].surname .. msg.fwd_from.last_name
                end
                if msg.fwd_from.real_last_name then
                    text = text .. langs['it'].surname .. msg.fwd_from.real_last_name
                end
                if msg.fwd_from.username then
                    text = text .. langs['it'].username .. '@' .. msg.fwd_from.username
                end
                ...
                if our_id ~= msg.fwd_from.peer_id then
                    ...
                    if msg.fwd_from.phone then
                        text = text .. langs['it'].phone .. '+' .. string.sub(msg.fwd_from.phone, 1, 6) .. '******'
                    end
                    ]]
                end
                text = text .. langs['it'].rank .. reverse_rank_table[get_rank(msg.fwd_from.peer_id, msg.to.id) + 1] ..
                langs['it'].date .. os.date('%c') ..
                langs['it'].otherInfo
...
local function run(msg, matches)
    ...
    if matches[1]:lower() == "getrank" or matches[1]:lower() == "rango" then
        ...
        elseif matches[2] then
            if string.match(matches[2], '^%d+$') then
                return langs['it'].rank .. reverse_rank_table[get_rank(matches[2], chat) + 1]
            else
                return resolve_username(string.gsub(matches[2], '@', ''), get_rank_by_username, { receiver = receiver, chat_id = chat })
            end
        else
            return langs['it'].rank .. reverse_rank_table[get_rank(msg.from.id, chat) + 1]
        end
    end
    ...
    if matches[1]:lower() == 'info' or matches[1]:lower() == 'sasha info' then
        ...
        elseif matches[2] then
            ...
            local text = langs['it'].info ..
            langs['it'].youAre
            if msg.from.title then
                text = text .. langs['it'].name .. msg.from.title
            end
            if msg.from.first_name then
                text = text .. langs['it'].name .. msg.from.first_name
            end
            if msg.from.real_first_name then
                text = text .. langs['it'].name .. msg.from.real_first_name
            end
            if msg.from.last_name then
                text = text .. langs['it'].surname .. msg.from.last_name
            end
            if msg.from.real_last_name then
                text = text .. langs['it'].surname .. msg.from.real_last_name
            end
            if msg.from.username then
                text = text .. langs['it'].username .. '@' .. msg.from.username
            end
            -- exclude bot phone
            if our_id ~= msg.from.id then
                --[[
                if msg.from.phone then
                    text = text .. langs['it'].phone .. '+' .. string.sub(msg.from.phone, 1, 6) .. '******'
                end
                ]]
            end
            local msgs = tonumber(redis:get('msgs:' .. msg.from.id .. ':' .. msg.to.id) or 0)
            text = text .. langs['it'].rank .. reverse_rank_table[get_rank(msg.from.id, chat) + 1] ..
            langs['it'].date .. os.date('%c') ..
            langs['it'].totalMessages .. msgs ..
            langs['it'].otherInfo
            ...
            text = text .. '\n🆔: ' .. msg.from.id ..
            langs['it'].youAreWriting
            if chat_type == 'user' then
                text = text .. ' 👤'
                if msg.to.first_name then
                    text = text .. langs['it'].name .. msg.to.first_name
                end
                if msg.to.real_first_name then
                    text = text .. langs['it'].name .. msg.to.real_first_name
                end
                if msg.to.last_name then
                    text = text .. langs['it'].surname .. msg.to.last_name
                end
                if msg.to.real_last_name then
                    text = text .. langs['it'].surname .. msg.to.real_last_name
                end
                if msg.to.username then
                    text = text .. langs['it'].username .. '@' .. msg.to.username
                end
                -- exclude bot phone
                if our_id ~= msg.to.id then
                    --[[
                    if msg.to.phone then
                        text = text .. langs['it'].phone .. '+' .. string.sub(msg.to.phone, 1, 6) .. '******'
                    end
                    ]]
                end
                text = text .. langs['it'].rank .. reverse_rank_table[get_rank(msg.to.id, chat) + 1] ..
                langs['it'].date .. os.date('%c') ..
                '\n🆔: ' .. msg.to.id
                return text
            elseif chat_type == 'chat' then
                text = text .. ' 👥' ..
                langs['it'].groupName .. msg.to.title ..
                langs['it'].users .. tostring(msg.to.members_num) ..
                '\n🆔: ' .. math.abs(msg.to.id)
                return text
            elseif chat_type == 'channel' then
                text = text .. ' 👥' ..
                langs['it'].supergroupName .. msg.to.title
                if msg.to.username then
                    text = text .. langs['it'].username .. "@" .. msg.to.username
                end
                text = text .. "\n🆔: " .. math.abs(msg.to.id)
    ...
    if matches[1]:lower() == 'grouplink' or matches[1]:lower() == 'sasha link gruppo' or matches[1]:lower() == 'link gruppo' and matches[2] then
        if is_admin1(msg) then
            ...
            if not group_link then
                return langs['it'].noLinkAvailable
            end
            return matches[2] .. '\n' .. group_link
        else
            return langs['it'].require_admin
