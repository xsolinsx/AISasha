    ...
    if msg.from.type == 'user' then
        ...
        if msgs > max_msg then
            ...
            if msg.to.type == 'user' then
                ...
                if msgs >= max_msg then
                    print("Pass2")
                    send_large_msg("user#id" .. msg.from.id, langs['it'].user .. "[" .. msg.from.id .. "]" .. langs['it'].blockedForSpam)
            ...
            if msg.to.type == 'chat' or msg.to.type == 'channel' then
                if username then
                    ...
                    send_large_msg(receiver, langs['it'].floodNotAdmitted .. "@" .. username .. "[" .. msg.from.id .. "]\n" .. langs['it'].statusRemoved .. " (SPAM)")
                else
                    savelog(msg.to.id, name_log .. " [" .. msg.from.id .. "] kicked for #spam")
                    send_large_msg(receiver, langs['it'].floodNotAdmitted .. langs['it'].name .. name_log .. "[" .. msg.from.id .. "]\n" .. langs['it'].statusRemoved .. " (SPAM)")
                end
            end
            -- incr it on redis
            ...
            if gbanspamonredis then
                if tonumber(gbanspamonredis) == 4 and not is_owner(msg) then
                    ...
                    send_large_msg("chat#id" .. msg.to.id, langs['it'].user .. "[ " .. name .. " ]" .. msg.from.id .. langs['it'].gbanned .. " (SPAM)")
                    send_large_msg("channel#id" .. msg.to.id, langs['it'].user .. "[ " .. name .. " ]" .. msg.from.id .. langs['it'].gbanned .. " (SPAM)")
                    local GBan_log = 'GBan_log'
                    local GBan_log = data[tostring(GBan_log)]
                    for k, v in pairs(GBan_log) do
                        log_SuperGroup = v
                        gban_text = langs['it'].user .. "[ " .. name .. " ] ( @" .. username .. " )" .. msg.from.id .. langs['it'].gbannedFrom "( " .. msg.to.print_name .. " ) [ " .. msg.to.id .. " ] (SPAM)"
