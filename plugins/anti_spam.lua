    ...
    if msg.from.type == 'user' then
        ...
        if msgs > max_msg then
            ...
            if msg.to.type == 'user' then
                ...
                if msgs >= max_msg then
                    print("Pass2")
                    send_large_msg("user#id" .. msg.from.id, langs.user .. "[" .. msg.from.id .. "]" .. langs.blockedForSpam)
            ...
            if msg.to.type == 'chat' or msg.to.type == 'channel' then
                if username then
                    ...
                    send_large_msg(receiver, langs.floodNotAdmitted .. "@" .. username .. "[" .. msg.from.id .. "]\n" .. langs.statusRemoved .. " (SPAM)")
                else
                    savelog(msg.to.id, name_log .. " [" .. msg.from.id .. "] kicked for #spam")
                    send_large_msg(receiver, langs.floodNotAdmitted .. langs.name .. name_log .. "[" .. msg.from.id .. "]\n" .. langs.statusRemoved .. " (SPAM)")
                end
            end
            -- incr it on redis
            ...
            if gbanspamonredis then
                if tonumber(gbanspamonredis) == 4 and not is_owner(msg) then
                    ...
                    send_large_msg("chat#id" .. msg.to.id, langs.user .. "[ " .. name .. " ]" .. msg.from.id .. langs.gbanned .. " (SPAM)")
                    send_large_msg("channel#id" .. msg.to.id, langs.user .. "[ " .. name .. " ]" .. msg.from.id .. langs.gbanned .. " (SPAM)")
                    local GBan_log = 'GBan_log'
                    local GBan_log = data[tostring(GBan_log)]
                    for k, v in pairs(GBan_log) do
                        log_SuperGroup = v
                        gban_text = langs.user .. "[ " .. name .. " ] ( @" .. username .. " )" .. msg.from.id .. langs.gbannedFrom "( " .. msg.to.print_name .. " ) [ " .. msg.to.id .. " ] (SPAM)"
